...
_WORDS_WITH_PERIODS = {"mr.", "dr.", "mrs.", "etc."}
_MAX_WORDS = 90  # proxy for max_tokens=128 from the original code


def _extract_sentences(text: str) -> list[str]:
    sentences = _SENTENCE_RE.findall(text.strip())
    result = []
    for s in sentences:
        s = s.strip()
        words = s.split()
        if (
            len(words) < 5
            or len(words) >= _MAX_WORDS
            or s[-1] not in ".!?"
            or words[-1].lower() in _WORDS_WITH_PERIODS
        ):
            continue
        result.append(s)
    return result
...
class TranslationCyclesEnv(vf.MultiTurnEnv):
    ...
    async def env_response(
        ...
        return [{"role": "user", "content": backward_prompt}]
